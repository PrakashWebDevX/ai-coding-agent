...
Nodes: ProblemReader -> Planner -> Generator -> Formatter -> PasteEditor -> [STOP, awaiting user]
Then, on user-triggered run: RunTest -> ErrorAnalyzer -> (Retry loop | Success) -> Memory -> Dashboard

The graph is split into two entry points because Run Tests is a user-triggered action
(per PRD: "User clicks Run Tests in dashboard"), not something the agent does autonomously.
"""
...
def build_solve_graph():
    """Graph 1: Problem -> Plan -> Generate -> Format -> Paste. Stops for user to click Run."""
